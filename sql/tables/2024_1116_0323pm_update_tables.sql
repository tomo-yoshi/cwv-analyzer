...
-- Create the PageSpeed Records table if it doesn't exist
create table if not exists public.pagespeed_records (
  id uuid not null default gen_random_uuid(),
  display_name text not null,
  url text not null,
  records jsonb not null,
...
create index if not exists pagespeed_records_project_id_idx on public.pagespeed_records(project_id);
create index if not exists pagespeed_records_profile_id_idx on public.pagespeed_records(profile_id);
...
begin
  ...
  execute format('alter table public.pagespeed_records enable row level security');
...
-- Create RLS policies for pagespeed_records if they don't exist
do $$
begin
  create policy "Users can view their own records" on public.pagespeed_records
    for select using (auth.uid() = profile_id);
    
  create policy "Users can insert their own records" on public.pagespeed_records
    for insert with check (auth.uid() = profile_id);
    
  create policy "Users can update their own records" on public.pagespeed_records
    for update using (auth.uid() = profile_id);
    
  create policy "Users can delete their own records" on public.pagespeed_records
    for delete using (auth.uid() = profile_id);
exception when duplicate_object then
  null;
end $$;